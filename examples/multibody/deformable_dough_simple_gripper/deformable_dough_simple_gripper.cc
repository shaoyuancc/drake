...
class GripperPositionControl : public systems::LeafSystem<double> {
 ...
 private:
  ...
  const double fingers_closed_time_{1.0};
  /* The time at which the gripper reaches the desired "lifted" state. */
  const double gripper_lifted_time_{2.0};
  const double hold_time_{4.0};
  /* The time at which the fingers reach the desired open state. */
  const double fingers_open_time_{5.0};
...
int do_main() {
  ...
  const std::string gripper_path = FindResourceOrThrow(
      "drake/examples/multibody/deformable_dough_simple_gripper/simple_gripper.sdf");
  ...
  const std::string dough_vtk = FindResourceOrThrow(
      "drake/examples/multibody/deformable_dough_simple_gripper/sphere_541.vtk");
  ...
  const double open_width = kL * 2.5; //kL * 1.5;
  const double closed_width = kL * 1.0; //kL * 0.4;
  const double lifted_height = 0.05; //0.18;
