 ...
 public:
  ...
  /* Constructs a GripperPositionControl system with the given parameters.
   ...
   @param[in] height       The height of the gripper in the lifted state.
                           (meters) */
  GripperPositionControl(double initial_height, double final_height)
      : initial_state_(initial_height),
        final_state_(final_height)
        {
    this->DeclareVectorOutputPort("gripper force", BasicVector<double>(1),
                                  &GripperPositionControl::SetAppliedForce);
    this->DeclareVectorInputPort("gripper state", BasicVector<double>(2));
  }
  ...
  void SetAppliedForce(const Context<double>& context,
    ...
    const VectorXd gripper_state =
        EvalVectorInput(context, GetInputPort("gripper state").get_index())
            ->get_value();
    /* There are 2 dofs in the state, corresponding to
     q_translate_joint, v_translate_joint */
    /* The positions of the translate joint*/
    const Vector1d measured_positions = gripper_state.head(1);
    /* The velocities of the translate joint and the left finger. */
    const Vector1d measured_velocities = gripper_state.segment<1>(1);
    const Vector1d desired_velocities(0);
    Vector1d desired_positions;
    const double t = context.get_time();
    if (t < descend_time_) {
        const double end_time = descend_time_;
    ...
    std::cout << "\ntime = " << t;
    std::cout << "\tmeasured position = " << measured_positions[0];
    std::cout << "\tmeasured velocity = " << measured_velocities[0];
    std::cout << "\tcommanded force = " << force[0];
    output->get_mutable_value() << force;
  }

  /* The time at which the fingers reach the final height. */
  const double descend_time_{1.0};
  const double ascend_time_{1.5};
  Vector1d initial_state_;
  Vector1d final_state_;
...
int do_main() {
  ...
  // station instead.
  /* Set up sphere manipulator. */
  Parser parser(&plant);
  const std::string gripper_path = FindResourceOrThrow(
      "drake/examples/multibody/deformable_dough_sphere_manipulator/sphere_manipulator.sdf");
  parser.AddModels(gripper_path);
  /* Add collision geometries. */
  const RigidTransformd X_BG = RigidTransformd::Identity();
  const Body<double>& body = plant.GetBodyByName("body");
  /* The size of the fingers is set to match the visual geometries in
   simple_gripper.sdf. */
  plant.RegisterCollisionGeometry(body, X_BG, Box(0.146, 0.0725, 0.049521),
                                  "body_collision",
  ...
  const std::string dough_vtk = FindResourceOrThrow(
      "drake/examples/multibody/deformable_dough_sphere_manipulator/sphere_541.vtk");
  ...
  auto dough_mesh = std::make_unique<Mesh>(dough_vtk, scale);
  /* Minor diameter of the torus inferred from the vtk file. */
  const double kL = 0.1 * scale;
  /* Set the initial pose of the torus such that its bottom face is touching the
   ground. */
  const RigidTransformd X_WB(Vector3<double>(0.0, 0.0, kL));
  auto dough_instance = std::make_unique<GeometryInstance>(
      X_WB, std::move(dough_mesh), "deformable_dough");
  ...
  dough_instance->set_proximity_properties(deformable_proximity_props);
  ...
  owned_deformable_model->RegisterDeformableBody(
      std::move(dough_instance), deformable_config, unused_resolution_hint);
